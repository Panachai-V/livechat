...
def home(request):
    ...
    {'urllogin':resolve_url('marialogin'),
    'urllogout':resolve_url('marialogout')})

def marialogin(request):
    ...
    if user is not None:
        ...
        print(resolve_url('mariaregister'))
        #show error
        return render(request,'login.html',
        {'error':'Your username or password are incorrect',
        'urlregister':resolve_url('mariaregister'),
        'urllogin':resolve_url('marialogin')})

def marialogout(request):
    # context = RequestContext(request)
    logout(request)
    # Redirect back to index page.
    return redirect('marialogin')

def mariaregister(request):
